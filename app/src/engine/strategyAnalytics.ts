...
function calculateLetterFrequencies(puzzles: Puzzle[]): LetterFrequency[] {
  ...
  // Count letter occurrences across all puzzles
  ...
  for (const puzzle of puzzles) {
    ...
    // Count which puzzles contain each letter
  ...
  // Convert to LetterFrequency array
  ...
  // Sort by occurrence rate (highest first), then by total count
...
function calculateCategoryAnalysis(puzzles: Puzzle[]): CategoryAnalysis[] {
  ...
  for (const [category, categoryPuzzles] of puzzlesByCategory.entries()) {
    ...
    // Find common patterns (simple implementation - look for common bigrams)
...
function findCommonPatterns(puzzles: Puzzle[]): string[] {
  ...
  for (const puzzle of puzzles) {
    ...
    // Check for common bigrams and trigrams
  ...
  // Return patterns that appear in at least 20% of puzzles
...
function generateRecommendations(
  ...
  // Top 5 consonants and vowels
  const topConsonants = consonantFreqs.slice(0, 5).map(f => f.letter);
  const topVowels = vowelFreqs.slice(0, 5).map(f => f.letter);

  // Optimal first guesses: R, S, T, L, N, E (classic Wheel of Fortune strategy)
  // But rank them by actual frequency in the dataset
  ...
  // Vowel buy threshold: recommend buying when you have enough to survive a bankrupt
  // With 8.33% bankrupt chance, recommend having at least $1000 before buying vowels
