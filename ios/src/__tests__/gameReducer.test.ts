...
function stateWithPuzzleInProgress(
  ...
  return {
    ...
    player: { currentRoundScore: 1500, totalScore: 3000, freePlay: false },
    roundCount: 2,
    ...overrides,
  };
}
...
describe('RANDOM_PUZZLE', () => {
  ...
  it('preserves totalScore when selecting random puzzle', () => {
    const state = stateWithPuzzleInProgress({
      ...
      player: { currentRoundScore: 1000, totalScore: 5000, freePlay: true },
    });
    const result = gameReducer(state, {
      type: 'RANDOM_PUZZLE',
      puzzles,
...
describe('SELECT_PUZZLE', () => {
  ...
  it('preserves totalScore', () => {
    const newPuzzle = makePuzzle({ id: 'selected' });
    const state = stateWithPuzzleInProgress({
      player: { currentRoundScore: 2000, totalScore: 8000, freePlay: false },
    });
    const result = gameReducer(state, {
      type: 'SELECT_PUZZLE',
      puzzle: newPuzzle,
