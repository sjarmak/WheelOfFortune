...
const TEST_PUZZLES: Puzzle[] = [
  ...
  {
    id: "6",
    phrase: "RUNNING A MARATHON",
    category: "WHAT_ARE_YOU_DOING",
    round_type: "MAIN",
  },
];
...
describe("calculateCategoryAnalysis", () => {
  ...
  it("includes letter frequencies per category", () => {
    const result = analyzePuzzlePack(TEST_PUZZLES);
    const phraseCategory = result.categoryBreakdown.find(
      (c) => c.category === "PHRASE",
    );
  ...
  it("assigns UNKNOWN category to puzzles without category", () => {
    ...
    const result = analyzePuzzlePack(puzzles);
    const unknown = result.categoryBreakdown.find(
...
describe("calculateWheelAnalysis", () => {
  it("uses iOS WHEEL_CONFIG with VALUE type (not CASH)", () => {
    ...
    const cashWedges = WHEEL_CONFIG.filter(
      (w) => (w.type as string) === "CASH",
    );
  ...
  it("probabilities sum to 100%", () => {
    ...
    const total =
      wa.cashProbability + wa.bankruptProbability + wa.loseTurnProbability;
  ...
  it("calculates correct probabilities for 24-wedge wheel", () => {
    ...
    const bankruptCount = WHEEL_CONFIG.filter(
      (w) => w.type === "BANKRUPT",
    ).length;
    const loseTurnCount = WHEEL_CONFIG.filter(
      (w) => w.type === "LOSE_TURN",
    ).length;
  ...
  it("calculates expectedValue as total cash divided by total wedges", () => {
    const result = analyzePuzzlePack(TEST_PUZZLES);
    const totalCash = WHEEL_CONFIG.filter((w) => w.type === "VALUE").reduce(
      (sum, w) => sum + w.value,
      0,
    );
  ...
  it("calculates avgCashValue as total cash divided by cash wedges", () => {
    ...
    expect(result.wheelAnalysis.avgCashValue).toBeCloseTo(
      totalCash / valueWedges.length,
      5,
    );
  });
});
