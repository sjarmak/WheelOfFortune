 ...
 */
export function calculateVowelRatio(letters) {
    if (letters.length === 0)
        return 0;
 ...
 */
export function countUniqueLetters(letters) {
    return new Set(letters).size;
}
/**
 * Count rare letters (J, Q, X, Z)
 */
export function countRareLetters(letters) {
    return letters.filter(l => 'JQXZ'.includes(l)).length;
}
/**
 * Validate a phrase
 */
export function validatePhrase(phrase, options = {}) {
    const { minLength = 3, maxLength = 100, maxNonLetterRatio = 0.3, } = options;
    ...
    if (lowerPhrase.includes('category') ||
