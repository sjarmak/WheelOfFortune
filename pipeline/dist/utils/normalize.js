...
export function normalizeCategory(raw) {
    ...
    // Direct match
    if (CANONICAL_CATEGORIES.includes(slug)) {
        return slug;
    }
    // Fuzzy / Mapping
    if (slug === 'FOOD_&_DRINK' || slug === 'FOOD_DRINK')
        return 'FOOD_AND_DRINK';
    if (slug === 'SAME_LETTER')
        return 'PHRASE'; // Mapping some specific ones or keep as OTHER
    if (slug.includes('THINGS'))
        return 'THING';
    if (slug.includes('PEOPLE'))
        return 'PERSON';
    if (slug.includes('PLACES'))
        return 'PLACE';
...
export function validatePhrase(phrase) {
    ...
    if (letters.length < 2)
        return { valid: false, reason: 'not_enough_letters' };
    const nonsense = /([A-Z])\1{3,}/; // 4 same chars in a row
    if (nonsense.test(phrase.toUpperCase()))
        return { valid: false, reason: 'nonsense_repeat' };
    return { valid: true };
}
export function getCleanLetters(phrase) {
