...
import type { RoundType } from './schema.js';
...
program
  ...
  .action(async (opts) => {
    try {
      const categoriesInclude = opts.categoriesInclude?.split(',').map((s: string) => s.trim().toUpperCase());
      const categoriesExclude = opts.categoriesExclude?.split(',').map((s: string) => s.trim().toUpperCase());
      const roundTypes = opts.roundTypes.split(',').map((s: string) => s.trim().toUpperCase()) as RoundType[];
