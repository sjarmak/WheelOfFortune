import { Category, CANONICAL_CATEGORIES } from '../schema.js';
 ...
 */
export function mapCategory(input: string | undefined): { category: Category; original?: string } {
  if (!input || input.trim() === '') {
  ...
  if (CANONICAL_CATEGORIES.includes(upperInput as Category)) {
    return { category: upperInput as Category };
  }
