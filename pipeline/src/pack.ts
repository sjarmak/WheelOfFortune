...
import { PackSchema, ReportSchema, type Pack, type Puzzle, type Report, type RoundType, type Category } from './schema.js';
 ...
 */
export async function ingestPuzzles(options: IngestOptions): Promise<IngestResult> {
  const {
    inputs,
    pack: packName,
    seed,
  ...
  interface ProcessedPuzzle {
    ...
    vowelRatio: number;
    difficulty: { score: number; reasons: string[] };
  }

  const processed: ProcessedPuzzle[] = [];
  ...
  // Build final puzzles
  const puzzles: Puzzle[] = finalPuzzles.map(p => ({
    id: p.id,
    phrase: p.phrase,
    category: p.category,
  ...
  const roundTypeDist: Record<string, number> = {};
...
function detectSourceType(filePath: string): string {
  ...
  if (filePath.endsWith('.txt')) return 'text_file';
